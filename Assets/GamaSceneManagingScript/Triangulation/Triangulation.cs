...
using ummisco.gama.unity.geometry.tools;
using UnityEngine;

namespace ummisco.gama.unity.geometry.triangulation
{
    public static class Triangulation
    {
        ...
        public static List<Triangle> TriangulateConcavePolygon(List<Vector3> points)
        {
            ...
            {
                int nextPos = GeometryTools.ClampListIndex(i + 1, vertices.Count);

                int prevPos = GeometryTools.ClampListIndex(i - 1, vertices.Count);
        ...
        private static void IsVertexEar(Vertex v, List<Vertex> vertices, List<Vertex> earVertices)
        {
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                if (vertices[i].isReflex)
                {
                    ...
                    if (GeometryTools.IsPointInTriangle(a, b, c, p))
                    {
                        hasPointInside = true;
